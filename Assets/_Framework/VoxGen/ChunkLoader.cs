...
namespace _Framework.VoxGen
{
    ...
    public class ChunkLoader : MonoBehaviour
    {
        ...
        {
            _voxelsBuffer = new ComputeBuffer(_voxels.Length, sizeof(float));
            _voxelsBuffer.SetData(_voxels);

            _triangleBuffer = new ComputeBuffer(256, sizeof(ulong));
            _triangleBuffer.SetData(TriangleTable);
            
            _counterBuffer = new ComputeBuffer(1, sizeof(int), ComputeBufferType.Counter);
            _counterBuffer.SetCounterValue(0);
            
            voxelShader.SetInts("Size", new int[] {width, height, length});
            voxelShader.SetInt("Budget", budget);
            voxelShader.SetBuffer(0, "Voxels", _voxelsBuffer);
            voxelShader.SetBuffer(0, "TriangleTable", _triangleBuffer);
            voxelShader.SetBuffer(0, "Counter", _counterBuffer);
            
            ComputeBuffer vertexBuffer = new ComputeBuffer(budget * 3, 3 * sizeof(float));
            ComputeBuffer indexBuffer = new ComputeBuffer(budget * 3, sizeof(int));
            ...
            voxelShader.SetBuffer(1, "Counter", _counterBuffer);
            voxelShader.Dispatch(1, 1, 1, 1);
            
            Vector3[] vertices = new Vector3[budget * 3];
            vertexBuffer.GetData(vertices);
            int[] indices = new int[budget * 3];
        ...
        public void FillVoxels(float perlinX, float perlinY)
        {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                {
                    /*float noise = Mathf.PerlinNoise((float)(x + perlinY * (width-1)) / (width * perlinScale),
                        (float)(z + perlinX * (length-1)) / (length * perlinScale));*/
                    float noise = Noisefunction(x + perlinY * (width - 1), z + perlinX * (length -1), Vector2.zero);
                    int perlinHeight = Mathf.RoundToInt(noise * height);
                    if (perlinHeight <= 0) perlinHeight = 2;
                    if (perlinHeight >= height) perlinHeight = height - 1;
                    for (int y = 0; y < perlinHeight - 1; y++)
                    {
                        _voxels[x, y, z] = 1f;
                    }
                    if (noise * height - perlinHeight > 0.1f)
                    {
                        _voxels[x, perlinHeight - 1, z] = noise * height - perlinHeight;
                    }
                }
            }
        }
        ...
        {
            float a = 0, noisesize = 0.5f, opacity = 1;

            for (int octaves = 0; octaves < 11; octaves++)
            {
                float xVal = (x / (noisesize * 360)) + Origin.x;
                float yVal = (y / (noisesize * 360)) - Origin.y;
            ...
            return a -= FallOffMap(x, y, 360, 40);
        }
