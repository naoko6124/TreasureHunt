...
public class VoxelCube : MonoBehaviour
{
    ...
    {
        FillVoxels(Random.Range(0.0f, 100.0f));

        voxelsBuffer = new ComputeBuffer(voxels.Length, sizeof(float));
        voxelsBuffer.SetData(voxels);

        triangleBuffer = new ComputeBuffer(256, sizeof(ulong));
        ...
        voxelShader.SetBuffer(0, "Voxels", voxelsBuffer);
        voxelShader.SetBuffer(0, "TriangleTable", triangleBuffer);
        voxelShader.SetBuffer(0, "Counter", counterBuffer);
    ...
    private void ConfigureMesh(int budget)
    {
        ...
        mesh.SetVertexBufferParams(budget * 3, vp, vn);
        mesh.SetIndexBufferParams(budget * 3, IndexFormat.UInt32);

        mesh.SetSubMesh(0, new SubMeshDescriptor(0, budget * 3), MeshUpdateFlags.DontRecalculateBounds);
    ...
    private void FillVoxels(float perlinRandom)
    {
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            {
                float noise = Mathf.PerlinNoise((float)x / (width * perlinScale) + perlinRandom,
                (float)z / (length * perlinScale));
                int perlinHeight = Mathf.RoundToInt(noise * height);
                if (perlinHeight <= 0) perlinHeight = 1;
    ...
    private static ulong [] triangleTable =
    {
